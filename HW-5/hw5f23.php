...
. useridf23 > text
. emailf23f23 > email field
. entryYearf23 > entry year > radio button(s) (use 2019,2020,2021,2022,2023,0 (0 label = other))
. birthYearf23 > use select box (1990 to now)
. gradDatef23 > Expected Graduation Date  (date field)
. csef23 > Are you a CSE major> use checkbox
. carf23 > Do you own a car > checkbox
. mobileDevicef23 > Number of mobile devices (including laptops, ipads, etc) you own > select (0-10)
. quotef23 > Favorite quote > text area (5 lines by 40 columns)
. quoteCitef23 > Source of quote (site, who said it, etc) >  url
. commandf23 > Hidden field with value = "submit"
...
//
	$FIELDS=array( "useridf23", "emailf23", "entryYearf23", "birthYearf23", "gradDatef23", "mobileDevicef23", "quotef23", "quoteCitef23", "commandf23");
	$err=0;
...
try {
    //connect to DB
    @$mysqli = new mysqli("10.32.1.92","383",$PASSWORD,"383");
    //
...
if ($cmd=="submit") {
	$uid = getVar("useridf23");
	$entryYear = getVar("entryYearf23");
	$birthYear = getVar("birthYearf23");
	$cse = getVar("csef23");
	$car = getVar("carf23");
	$mobileDevices = getVar("mobileDevicef23");
    if (!is_numeric($mobileDevices))
      $mobileDevices = -1;
	$quote = getVar("quotef23");
	$quoteCite = getVar("quoteCitef23");
